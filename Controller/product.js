...
exports.CreateProduct = async(req, res) => {
    try {
        ...
        if (!user) return res.status(400).json('no user exists');
...
exports.GetProductByID = (req, res) => {
    ...
    ProdSchema.findOne({ $and: [{ _id: id }, { email: req.email }] })
        .then((result) => {
            if (!result) return res.status(400).json('no product exits');
...
exports.UpdateProductByID = async(req, res) => {
    try {
        let id = req.params.id;
        let validate = validationResult(req);
        if (!validate.isEmpty()) return res.status(400).json(validate.array());
...
exports.DeleteProductByID = async(req, res) => {
    try {
        let id = req.params.id;
        let user = await UserSchema.findOne({ email: req.email }).populate(
            'product'
        );

        if (!user) return res.status(400).json('no user exists');

        await ProdSchema.deleteOne({
            $and: [{ _id: id }, { email: req.email }],
        });
